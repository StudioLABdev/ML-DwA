...
class DWAMetrics(DetMetrics):
    ...
    def process(self, tp, attr_preds, attr_targets, conf, pred_cls, target_cls):
        """Process predicted results for object detection and update metrics."""
        results = ap_per_class(tp,
                               conf,
                               pred_cls,
    ...
    def keys(self):
        """Returns a list of keys for accessing specific metrics."""
        return ['metrics/precision(B)', 'metrics/recall(B)', 'metrics/mAP50(B)', 'metrics/mAP50-95(B)',
                 'metrics/accuracy(A)', 'metrics/precision(A)', 'metrics/recall(A)', 'metrics/F1-score(A)']

    def mean_results(self):
        """Calculate mean of detected objects & return precision, recall, mAP50, and mAP50-95."""
        return self.box.mean_results() + self.attr_results

    def class_result(self, i):
        """Return the result of evaluating the performance of an object detection model on a specific class."""
        return self.box.class_result(i)
    
    def attributes_metrics(self, attr_preds, attr_targets):
        attr_targets = attr_targets > 0.5
        acc = (attr_preds == attr_targets).mean()
        prec = (attr_preds * attr_targets).sum() / (attr_preds.sum() + 1e-16)
